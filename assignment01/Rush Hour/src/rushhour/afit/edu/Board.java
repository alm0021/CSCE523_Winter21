...
public class Board
{
    ...
    /*
     * Outputs a string representation of board. Useful as a hashkey
     *
    ...
    public String toString() {
      ...
      for ( int i = 0; i < Board.BOARD_SIZE+1; i++ )
        for ( int j = 0; j < Board.BOARD_SIZE; j++ )
          outString = outString.concat(String.valueOf(theBoard[i][j]));
      return outString;
    }
      ...
      for ( int i = Board.BOARD_SIZE-1; i >= 0; i-- ) {
        for ( int j = 0; j < Board.BOARD_SIZE+1; j++ ) {
          if (theBoard[j][i] == -1)
            ...
            System.out.print(piece_list[theBoard[j][i]].name + " ");
        }
        System.out.println();
      }
  }
